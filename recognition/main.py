...
def create_tables():
    ...
    try:
        ...
        if 'photo' not in columns:
            cursor.execute("ALTER TABLE faces ADD COLUMN photo TEXT")
...
def get_known_faces():
    """
    Fetch all known face embeddings, their associated names, and photo paths from the database.
    Returns:
        - names (list): List of names of registered individuals.
        - embeddings (list): List of numpy arrays containing face embeddings.
        - photos (list): List of photo paths.
    """
    default_photo_path = "./NoFedImage/default.jpg"  # Default image
    conn = sqlite3.connect("../database/attendance.db")
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT name, embedding, photo FROM faces")
        data = cursor.fetchall()
        names = [row[0] for row in data]
        embeddings = [np.array(row[1].split(','), dtype=float) for row in data]
        photos = [
            row[2] if row[2] is not None and os.path.exists(row[2]) else default_photo_path
            for row in data
        ]
        return names, embeddings, photos
    finally:
        conn.close()
...
def register_face(name=None):
    ...
    print("Camera initialize                                                                            d. Position yourself. Capturing in 4 seconds...")
    time.sleep(4)
